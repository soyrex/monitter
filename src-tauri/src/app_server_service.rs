...
impl Service {
    ...
    pub(crate) fn replace_acp_transport(
        ...
        // handshake instead of attempting a competing reserve_run.  The
        // handshake is an active operation even though its preceding durable
        // prompt was interrupted; this also lets Stop fence this exact owner.
        replacement.begin_run()?;
    ...
    pub(crate) fn app_server_message(
        ...
        if self.is_internal_admin_task(task_id) {
            // Do not let a late frame from an old resident owner leak into
            // the broker after a cold wake. It remains transcript-invisible,
            // but has the same ownership/turn fence as ordinary messages.
            self.app_server_mutate(task_id, control, Some(turn_id), |_, _| Ok(()))?;
            self.admin_turn_broker
                .capture_app_server_item(task_id, item_id, text);
            return Ok(());
        }
    ...
    pub(crate) fn app_server_event(
        ...
        // The resident Monitter Admin lane is activity-invisible, but its
        // native session is durable state needed for a later cold wake.
        if self.is_internal_admin_task(task_id) {
            let native = parsed.native_session_id.as_deref();
            self.app_server_mutate(task_id, control, turn_id, |data, runs| {
                if let Some(native) = native {
                    ...
                    task.native_session_id = Some(native.into());
                    ...
                    {
                        return Err(
                            "This native Codex session already has another active Monitter writer."
                                .into(),
                        );
                    }
    ...
    /// A `turn/steer` acknowledgement makes the locally durable pending
    /// record a real user message in the active native turn. The record is
    /// intentionally retained until this confirmation: a rejection can still
    /// safely fall back to the ordinary FIFO queue.
    pub(crate) fn app_server_steer_accepted(
        self: &Arc<Self>,
        task_id: &str,
        control: &Arc<RunControl>,
        turn_id: &str,
        ...
        let accepted = self.app_server_mutate(task_id, control, Some(turn_id), |data, _| {
            let message_index = data
                ...
                .ok_or("The pending steered message is no longer available.")?;
        ...
        if let Err(error) = accepted {
            // Completion can race an acknowledgement. A message is only
            // considered steered after the durable mutation above succeeds.
            self.app_server_steer_rejected(task_id, control, queued_message_id, &error);
        }
    }

    /// Return an unconfirmed steer to the normal FIFO queue. This covers an
    /// unsupported active turn, an `expectedTurnId` race, and a transport that
    /// became unavailable before Monitter could persist acceptance.
    pub(crate) fn app_server_steer_rejected(
        self: &Arc<Self>,
        task_id: &str,
        control: &Arc<RunControl>,
        queued_message_id: &str,
    ...
    pub(crate) fn complete_app_server_turn(
        ...
        if status == "completed" {
            // The task is still running here: no next accepted turn can
            // dispatch until completion is saved below. Only infrastructure
            // from a transport that has never executed tools can be adopted.
            let _ = control.refresh_runtime_process_baseline_if_no_tool_work();
        }
        // The resident Monitter Admin lane keeps its owner and native writer
        // claim while idle. Its completion only updates durable task state
        // and resolves the process-local caller; GC releases the owner later.
        if self.is_internal_admin_task(task_id) {
            ...
            if normalized == "completed" {
                ...
                self.admin_turn_broker.cancel_with_error(
                    task_id,
                    error.unwrap_or_else(|| "Monitter Admin turn ended before replying.".into()),
                );
            }
