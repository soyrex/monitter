...
const updateAnonymous=(text)=>console.log(JSON.stringify({jsonrpc:'2.0',method:'session/update',params:{sessionId:session,update:{sessionUpdate:'agent_message_chunk',content:{type:'text',text}}}}));
const messageDone=()=>console.log(JSON.stringify({jsonrpc:'2.0',method:'session/update',params:{sessionId:session,update:{sessionUpdate:'agent_message_done'}}}));
...
#[test]
fn completed_acp_message_survives_a_later_transport_failure() {
    let fixture = fixture_with_args("text-done-fails", vec!["text-done-fails".into()]);
    let agent = fixture.snapshot().unwrap().agents.remove(0);
    let task = fixture
        .create_task(CreateTaskInput {
            agent_id: agent.id,
            title: "ACP message-boundary fixture".into(),
            native_session_id: None,
            parent_task_id: None,
            channel_id: None,
            project_id: None,
    ...
    let accepted = fixture
        .accept_send(task.id.clone(), "test boundaries".into(), vec![])
        .unwrap()
        .unwrap();
    ...
    assert_eq!(messages[0].text, "finished progress");
    assert_eq!(messages[0].stream_status.as_deref(), Some("complete"));
    assert_eq!(messages[1].text, "unfinished progress");
    assert_eq!(messages[1].stream_status.as_deref(), Some("interrupted"));
}
