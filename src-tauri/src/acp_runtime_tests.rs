...
fn acp_collaboration_uses_scoped_stdio_server_and_revokes_on_stop() {
    for (enabled, expected) in [
        ...
        if let Some(control) = fixture.runs.lock().unwrap().tasks.get(&task.id).cloned() {
