...
impl Fixture {
    ...
    fn stop(&self) {
        self.service.cancel(&self.task.id).unwrap();
...
fn ssh_codex_dispatches_app_server_and_reuses_resident_process() {
    ...
    let helper = rpc.iter().find(|r| r["method"] == "thread/start").unwrap()["params"]["config"]
        ["mcp_servers.monitter.args"][0]
        .as_str()
        .unwrap();
    assert!(std::path::Path::new(helper).exists());
    f.stop();
    let deadline = Instant::now() + Duration::from_secs(8);
    while std::path::Path::new(helper).exists() && Instant::now() < deadline {
        thread::sleep(Duration::from_millis(20));
    }
    assert!(!std::path::Path::new(helper).exists());
}
