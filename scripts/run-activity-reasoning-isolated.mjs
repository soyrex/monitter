...
try {
  ...
  await expect(pending.locator('.animated-title').first()).toHaveAttribute('aria-busy', 'true');
  await expect(pending.locator('.animated-title').first()).toHaveAttribute('title', 'In progress');
  ...
  const imageActivity = page.locator('.activity').filter({ hasText: 'Viewed image: cargo.png' });
  await expect(imageActivity.getByRole('button', { name: /Viewed image: cargo\.png/ })).toBeVisible();
  await imageActivity.locator('.activity-trigger').click();
  const imagePreview = imageActivity.getByRole('button', { name: 'View full-size cargo.png' });
  await expect(imagePreview).toBeVisible();
  await expect(imagePreview.getByAltText('Preview of cargo.png')).toHaveAttribute('src', /^data:image\/png;base64,/);
  await imagePreview.click();
  await expect(page.getByRole('dialog', { name: 'cargo.png' })).toBeVisible();
