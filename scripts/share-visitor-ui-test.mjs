...
const server = await createServer({
  ...
  resolve: {
    alias: [
      {
        find: "$lib/controller/remote-client",
        replacement: resolve(root, "scripts/share-visitor-session-fixture.mjs"),
      },
...
const entry = {
  ...
  handle: (request, response, next) => {
    if (request.url === "/" || request.url?.startsWith("/share")) {
      ...
      response.end(
        '<div id="app"></div><script type="module" src="/scripts/share-visitor-ui-entry.js"></script>',
      );
    } else next();
  },
};
...
const task = async (browserType) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Shared design chat" }),
  ).toBeVisible();
  ...
  await expect(page.getByLabel("Read-only chat details")).toContainText(
    "Shared chat only",
  );
  await page.getByRole("button", { name: "Chat details", exact: true }).click();
  ...
  await expect(page.getByLabel("Attached files")).toContainText("upload.txt");
  await page.getByRole("button", { name: "Send" }).click();
  await expect
    .poll(() => page.evaluate(() => window.__shareTest.sendStarted))
    .toBe(true);
  ...
  await page.getByLabel("Message", {exact:true}).fill("Same text");
  await page.evaluate(() => {
    window.__shareTest.releaseSend = undefined;
  });
  await page.getByRole("button", { name: "Send" }).click();
  await expect
    .poll(() => page.evaluate(() => window.__shareTest.sendStarted))
    .toBe(true);
  await expect(page.locator(".delivery", { hasText: "Sending…" })).toBeVisible();
  await page.getByLabel("Message", {exact:true}).fill("Follow-up draft");
  await expect(page.getByLabel("Message", {exact:true})).toHaveValue("Follow-up draft");
  await page.evaluate(() => window.__shareTest.releaseSend());
  await expect(page.getByText("Same text", { exact: true })).toHaveCount(2);
  await expect(page.getByText("Sent", { exact: true })).toHaveCount(0);
  await page.evaluate(() => {
    window.__shareTest.failNext = true;
    window.__shareTest.sendStarted = false;
  });
  await page.getByLabel("Message", {exact:true}).fill("Same text");
  await page.getByRole("button", { name: "Send" }).click();
  await expect
  ...
  await expect(page.getByText("Not confirmed", { exact: true })).toBeVisible();
  await page.getByLabel("Message", {exact:true}).fill("Another text");
  await page.evaluate(() => {
    window.__shareTest.releaseSend = undefined;
  });
  await page.getByRole("button", { name: "Send" }).click();
  ...
  for (const viewport of [
    ...
    if (viewport.width > 640) {
      expect(before.window.width).toBe(1180);
      expect(before.window.x).toBe((viewport.width - 1180) / 2);
  ...
  await page.screenshot({
    path: `/tmp/monitter-share-dark-desktop-${browserType.name()}.png`,
  });
  await page.evaluate(() => {
    const root = document.documentElement;
    root.style.setProperty("--soft", "#dfd8cb"); root.style.setProperty("--code", "#e9e3d8"); root.style.setProperty("--accent-ink", "#87461f"); root.style.setProperty("--paper", "#fbf8f2");
    root.style.setProperty("--sidebar", "#e9e3d8");
    root.style.setProperty("--panel", "#f2ede3");
    root.style.setProperty("--line", "#c7c0b3");
    root.style.setProperty("--ink", "#262119");
    root.style.setProperty("--muted", "#665f56");
  });
  await page.screenshot({ path: `/tmp/monitter-share-light-desktop-${browserType.name()}.png` });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.evaluate(style => document.documentElement.setAttribute("style", style), darkStyle);
  await page.screenshot({ path: `/tmp/monitter-share-dark-mobile-${browserType.name()}.png` });
  await browser.close();
};
try {
  await task(chromium);
  await task(webkit);
  console.log(
    "Share visitor UI passed Chromium and WebKit mobile send checks and desktop/mobile pinned-layout checks.",
  );
} finally {
  await server.close();
}
