...
try{
  ...
  await page.getByRole('button',{name:'Resume session',exact:true}).click();await expect(page.locator('.composer-footer button[aria-label]').last()).toHaveAccessibleName('Starting task');
  await page.evaluate(()=>window.__MONITTER_QA__.release());await expect.poll(()=>calls().then(c=>c.length)).toBe(1);
  expect((await calls())[0].args.taskId).toBe('resume-chat');await expect(input).toHaveValue('Keep my draft');await expect(page.getByRole('button',{name:'Resume session',exact:true})).toHaveCount(0);
  ...
  await input.fill('A draft after failure');await page.getByRole('button',{name:'Resume session',exact:true}).click();await expect(page.getByText('Saved session is unavailable',{exact:true})).toBeVisible();await expect(input).toHaveValue('A draft after failure');
