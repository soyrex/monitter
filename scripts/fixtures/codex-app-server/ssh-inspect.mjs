...
if (process.env.MONITTER_TOKEN) {
  ...
  const result = await fetch(endpoint, {
    method: 'POST', headers: { Authorization: `Bearer ${process.env.MONITTER_TOKEN}`, 'Content-Type': 'application/json', Accept: 'application/json, text/event-stream' },
    body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'ping' }), signal: AbortSignal.timeout(3000),
  });
...
readline.createInterface({ input: process.stdin, crlfDelay: Infinity }).on('line', (line) => {
  ...
  if (rpc.method === 'thread/start' || rpc.method === 'thread/resume') {
    ...
    if (process.env.MONITTER_TOKEN) {
      assert.equal(rpc.params.config['mcp_servers.monitter.url'], process.env.MONITTER_ENDPOINT);
      assert.equal(rpc.params.config['mcp_servers.monitter.bearer_token_env_var'], 'MONITTER_TOKEN');
    }
  }
