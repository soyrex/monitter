...
try{
 ...
 const chrome=await page.evaluate(()=>({root:document.documentElement.style.backgroundColor,body:document.body.style.backgroundColor,blur:getComputedStyle(document.querySelector('.app-shell')).backdropFilter}));expect(chrome.root).toBe('transparent');expect(chrome.body).toBe('transparent');expect(chrome.blur).toContain('blur(22px)');
