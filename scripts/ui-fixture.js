...
(() => {
  ...
  window.__MONITTER_BRIDGE__ = {
    ...
    clearTaskContext:async taskId=>{record('clearTaskContext',{taskId});const task=state.tasks.find(t=>t.id===taskId);if(!task)throw Error('Task was not found.');task.nativeSessionId=null;task.status='idle';state.messages.push({id:crypto.randomUUID(),taskId,role:'system',text:'Context Cleared',createdAt:Date.now(),attachments:[]});notify();return copy();},
    ...
    cancelTask:async taskId=>{record('cancelTask',taskId);const task=state.tasks.find(t=>t.id===taskId);if(task.status!=='running')throw Error('Task is not running.');const createdAt=Date.now();task.status='interrupted';state.messages.push({id:crypto.randomUUID(),taskId,role:'system',text:'You cancelled this run.',createdAt,attachments:[]});state.events.push({id:crypto.randomUUID(),taskId,kind:'status',title:'You cancelled this run.',detail:'',createdAt});notify();return copy();},
