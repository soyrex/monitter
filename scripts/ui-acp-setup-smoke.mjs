  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      window.__MONITTER_BRIDGE__.verifyAcpAgent = async (hostId, launch) => {
        ...
        return {protocolVersion:1,agentName:'Fixture agent',agentVersion:'1.0',loadSession:true,resumeSession:false,image:false,audio:false,embeddedContext:false};
      };
    ...
    const picker = settings.getByRole('region',{name:'ACP agent setup'});
    ...
    await expect(picker.getByLabel('ACP executable',{exact:true})).toHaveValue('/test/local/gemini');
    await expect(picker.getByLabel('ACP argument 1',{exact:true})).toHaveValue('--acp');
    expect(await page.evaluate(() => window.__ACP_VERIFY_CALLS__)).toEqual([]);
    await activate(picker.getByRole('button',{name:'Verify ACP connection',exact:true}));
    await expect(picker.getByText('ACP v1 verified',{exact:false})).toBeVisible();
    expect(await page.evaluate(() => window.__ACP_VERIFY_CALLS__)).toEqual([{hostId:'local',launch:{command:'/test/local/gemini',args:['--acp']}}]);
    await picker.getByLabel('Search ACP agents').fill('Pi');
    await expect(picker.getByRole('button',{name:/Gemini CLI Detected/})).toHaveCount(0);
    await expect(picker.getByRole('button',{name:/Pi \(ACP bridge\) Not found/})).toBeVisible();
    await activate(picker.getByRole('button',{name:/Custom ACP agent/}));
    await expect(picker.getByText('ACP v1 verified',{exact:false})).toHaveCount(0);
    await picker.getByLabel('ACP executable',{exact:true}).fill('/path with spaces/custom-agent');
    await activate(picker.getByRole('button',{name:'Add argument'}));
    await picker.getByLabel('ACP argument 1',{exact:true}).fill('literal $(value) with spaces');
    ...
    await expect(picker.getByText('ACP v1 verified',{exact:false})).toHaveCount(0);
    await picker.getByLabel('Search ACP agents').fill('');
    await expect(picker.getByRole('button',{name:/Gemini CLI Detected/})).toContainText('/test/remote/gemini');
    await activate(picker.getByRole('button',{name:/Gemini CLI Detected/}));
    await expect(picker.getByLabel('ACP executable',{exact:true})).toHaveValue('/test/remote/gemini');
