...
try {
  ...
  const scoutRecipient = page.locator('.recipient-picker').getByRole('button', {name:'Scout',exact:true});
  if (await scoutRecipient.getAttribute('aria-pressed') !== 'true') await scoutRecipient.click();
  ...
  await page.getByRole('button',{name:'Stop current task',exact:true}).click();
  await page.getByLabel('Task message',{exact:true}).fill('A draft that survives archiving');
  await page.locator('.task-row').filter({hasText:'Palette archive target'}).hover();
  ...
  await page.getByRole('menuitem',{name:'Archived chats',exact:true}).click();
  dialog=page.getByRole('dialog',{name:'Archived chats',exact:true});
  await dialog.getByLabel('Search archived chats',{exact:true}).fill('Palette archive target');
  const archivedRow=dialog.locator('article').filter({hasText:'Palette archive target'});
  ...
  await page.getByRole('menuitem',{name:'Archived chats',exact:true}).click();
  dialog=page.getByRole('dialog',{name:'Archived chats',exact:true});
  await dialog.getByRole('button',{name:'Permanently delete Palette archive target',exact:true}).click();
  let confirm=page.getByRole('dialog',{name:'Permanently delete archived chat',exact:true});
  await expect(confirm).toContainText('Native session files will be kept');
  await confirm.getByRole('button',{name:'Cancel',exact:true}).click();
  expect(await page.evaluate(id=>window.__MONITTER_QA__.snapshot().tasks.some(t=>t.id===id),archivedId)).toBe(true);
  await page.evaluate(id=>{const qa=window.__MONITTER_QA__,s=qa.snapshot();s.deletionPreview={[id]:{supported:true,reason:'',files:['/tmp/native/a.jsonl','/tmp/native/b.jsonl']}};s.deleteFailure={[id]:'QA deletion failure'};qa.setSnapshot(s);},archivedId);
  await dialog.getByRole('button',{name:'Permanently delete Palette archive target',exact:true}).click();
  confirm=page.getByRole('dialog',{name:'Permanently delete archived chat',exact:true});
  await confirm.getByRole('switch',{name:'Also remove native session files from disk',exact:true}).check();
  await expect(confirm).toContainText('/tmp/native/a.jsonl');
  await confirm.getByRole('button',{name:'Delete permanently',exact:true}).click();
  await expect(confirm).toContainText('QA deletion failure');
  expect(await page.evaluate(id=>window.__MONITTER_QA__.snapshot().tasks.some(t=>t.id===id),archivedId)).toBe(true);
  await page.evaluate(id=>{const qa=window.__MONITTER_QA__,s=qa.snapshot();delete s.deleteFailure[id];qa.setSnapshot(s);},archivedId);
  await confirm.getByRole('button',{name:'Delete permanently',exact:true}).click();
  await expect.poll(()=>page.evaluate(id=>window.__MONITTER_QA__.snapshot().tasks.some(t=>t.id===id),archivedId)).toBe(false);
  results.push('archive-first restore and permanent deletion preserve native files by default and surface failures');
  await page.getByRole('dialog',{name:'Archived chats',exact:true}).getByRole('button',{name:'Close',exact:true}).click();
