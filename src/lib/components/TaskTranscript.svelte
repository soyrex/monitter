<script lang="ts">
  ...
  import { ArrowRightLeft, Check, CircleStop, Inbox, Maximize2, MoreHorizontal, Pencil, Share2, Terminal } from '@lucide/svelte';
      ...
      <TranscriptVirtualList
        ...
        {#snippet children(item, _index)}
          ...
          {:else if item.type === 'tool-group'}
            ...
            {#if inlineSubagent}<div class="subagent-inline"><UnifiedSubagentItem item={inlineSubagent} onclick={onOpenSubagent}/></div>{:else}<RunActivity active={active && !transcriptBuffer.held()} events={item.values} onloaddetail={onLoadFullEventDetail} compressed={display.settings.compressToolCalls === true} processTree running={displayTask.status === 'running'}/>{/if}
...
{#if showHeader && !displayMailInboxes.length}<div class="conversation-head task-heading pane-task-header">
...
<section class="conversation">
    ...
    {#if displayMailInboxes.length}
      <div class="mail-flip-stage" class:flipped={!inboxView}>
        <div class="mail-flip-inner">
          <div class="mail-flip-face mail-flip-front">
            <div class="mail-inbox-surface" aria-label="Mail inbox">
              {#each displayMailInboxes as inbox (inbox.id)}<MailTriageBatch batch={inbox} taskId={displayTask.id} fullHeight={displayMailInboxes.length === 1} onchat={() => { inboxView = false; transcriptBuffer.setFollowing(true); }}/>{/each}
            </div>
          </div>
          <div class="mail-flip-face mail-flip-back">
            <button class="mail-flip-back-toggle" type="button" aria-label="Return to inbox" title="Return to inbox" onclick={() => { inboxView = true; transcriptBuffer.setFollowing(true); }}><Inbox size={14}/>Inbox</button>
            {@render transcriptChat()}
          </div>
        </div>
      </div>
    {:else}
...
<style>
  ...
  .mail-flip-stage { position:relative; flex:1; min-width:0; min-height:0; perspective:1400px; }
  .mail-flip-inner { position:relative; width:100%; height:100%; min-height:0; transform-style:preserve-3d; transition:transform .55s cubic-bezier(.2,.7,.2,1); }
  .mail-flip-stage.flipped .mail-flip-inner { transform:rotateY(180deg); }
  .mail-flip-face { position:absolute; inset:0; display:flex; min-width:0; min-height:0; flex-direction:column; backface-visibility:hidden; -webkit-backface-visibility:hidden; }
  .mail-flip-face.mail-flip-back { transform:rotateY(180deg); }
  .mail-flip-back-toggle { position:absolute; z-index:3; top:10px; right:12px; display:inline-flex; align-items:center; gap:5px; min-height:29px; padding:0 9px; border:1px solid var(--line); border-radius:7px; color:var(--muted); background:color-mix(in srgb,var(--panel) 88%,transparent); font:500 calc(10px * var(--interface-font-ratio,1)) var(--mono); }
  .mail-flip-back-toggle:hover,.mail-flip-back-toggle:focus-visible { color:var(--ink); background:var(--soft); }
  .mail-flip-back :global(.message-pane) { height:100%; }
