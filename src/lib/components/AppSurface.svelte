<script lang="ts">
  ...
  async function saveAgent() {
    ...
    if(await run(()=>bridge.saveAgent(submitted),'Agent saved.')){delete agentEdits[oldId];agentDraft=JSON.parse(JSON.stringify(snapshot?.agents.find(agent=>agent.id===submitted.id)??submitted));void refreshCodexAccounts();}
  }
