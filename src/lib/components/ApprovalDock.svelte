...
{#if request}
  <section class="approval-dock" aria-label="Pending approvals" data-request-id={request.id}>
    ...
    <div class="dock-body">
      ...
      {#if !request.input && request.sessionScope === 'file_changes'}<p class="scope-note">Session approval covers every file edit in this chat until its harness stops. Commands and other permissions still ask separately.</p>{/if}
      {#if !request.input && request.rememberable === true}<p class="scope-note">The exact-action option is saved for this agent, host and folder. Revoke it in Settings → Approvals.</p>{/if}
    ...
    <div class="dock-actions">
      ...
      {:else}<button class="approve" disabled={disabled || resolving || !onresolve} onclick={() => onresolve?.(request, 'approve_once')}><Check size={17}/>Approve once</button>
        {#if request.sessionScope === 'file_changes'}<button class="session" disabled={disabled || resolving || !onresolve} onclick={() => onresolve?.(request, 'approve_session')}><Check size={17}/>Allow all edits this session</button>{/if}
        {#if request.rememberable === true}<button class="always" disabled={disabled || resolving || !onresolve} onclick={() => onresolve?.(request, 'approve_always')}><Check size={17}/>Always allow exact action</button>{/if}
      {/if}
