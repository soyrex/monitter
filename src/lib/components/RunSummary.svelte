<script lang="ts">
  import { GitBranch, Play, SquareTerminal } from '@lucide/svelte';
...
<style>.run-summary{display:grid;gap:9px;padding:11px;min-width:0}.git{display:grid;grid-template-columns:auto minmax(0,1fr);gap:6px;padding:6px;border:1px solid var(--line);border-radius:5px;text-align:left;min-width:0}.git span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.git small{grid-column:2;color:var(--muted);font-size:calc(10px * var(--interface-font-ratio, 1));white-space:normal}.processes{border-top:1px solid var(--line);padding-top:11px}.processes h3{margin:2px 0 5px;font-size:calc(10px * var(--interface-font-ratio, 1));color:var(--muted)}.processes p{display:flex;align-items:center;gap:5px;margin:3px 0;font-size:calc(11px * var(--interface-font-ratio, 1));min-width:0;overflow-wrap:anywhere}.empty{color:var(--muted)}</style>
