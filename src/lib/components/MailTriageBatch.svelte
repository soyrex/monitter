<script lang="ts">
  ...
  let error = $state('');
  let metricsCard = $state<MailCard | null>(null);
  let metricsAnchor = $state<HTMLElement | null>(null);
  ...
  const metric = (value: number | undefined) => value ? `${value}%` : 'Not retained';
  const cost = (value: number | null) => value === null ? 'Not reported' : `$${(value / 1_000_000).toFixed(6)}`;
  const inboxGroup = (item: MailCard) => item.confidence < 60 || item.replyRequired === 'unclear'
    ? 'Needs review'
    : item.importance === 'critical' || item.importance === 'high' || item.replyRequired === 'yes' || ['action_request', 'decision_needed', 'scheduling'].includes(item.intent) || ['reply', 'schedule', 'delegate', 'track'].includes(item.suggestedAction)
      ? 'Needs action'
      : 'For reference';

  function showMetrics(event: MouseEvent, card: MailCard) {
    event.preventDefault();
    event.stopPropagation();
    metricsAnchor = event.currentTarget as HTMLElement;
    metricsCard = card;
  }
  function closeMetrics() { metricsCard = null; metricsAnchor = null; }

  function stopPoll() { if (poll) clearTimeout(poll); poll = null; }
  function close() { generation += 1; stopPoll(); closeMetrics(); selected = null; detail = null; loading = false; error = ''; }
      ...
      <article class="mail-expanded" role="region" aria-label={`Email: ${selected.subject || 'No subject'}`}>
        <header class="expanded-row">
          ...
          <span class="score-stack">
            ...
            <button class="metrics-button" aria-label={`Show Jev classifications for ${selected.subject || 'email'}`} onclick={(event) => showMetrics(event, selected!)}><Info size={13}/></button>
          </span>
    ...
    {:else}
        ...
        <section class="mail-group" aria-label={group.label}>
          ...
          <div class="mail-list">
            {#each group.items as item (item.id)}
              <article class:new-mail={item.isNew} class="mail-card">
                <button class="mail-card-open" onclick={() => void open(item)} aria-label={`Open ${item.subject || 'email'} from ${item.from}`}></button>
                <span class="score-stack">
                  <span class={`importance ${importanceClass(item.importance)}`}>{item.importanceScore}</span>
                  <button class="metrics-button" aria-label={`Show Jev classifications for ${item.subject || 'email'}`} onclick={(event) => showMetrics(event, item)}><Info size={13}/></button>
                </span>
                <span class="mail-copy">
      ...
      {#if historyItems.length}
        <details class="mail-history"><summary>No longer matching <span>{historyItems.length}</span></summary><div class="mail-list history-list">
          {#each historyItems as item (item.id)}<article class="mail-card"><button class="mail-card-open" onclick={() => void open(item)} aria-label={`Open ${item.subject || 'email'} from ${item.from}`}></button><span class="score-stack"><span class={`importance ${importanceClass(item.importance)}`}>{item.importanceScore}</span><button class="metrics-button" aria-label={`Show Jev classifications for ${item.subject || 'email'}`} onclick={(event) => showMetrics(event, item)}><Info size={13}/></button></span><span class="mail-copy"><span class="mail-row"><strong>{item.subject || '(no subject)'}</strong><time>{received(item.receivedAt)}</time></span><span class="sender">{item.from}</span></span><ExternalLink class="open-icon" size={14} aria-hidden="true"/></article>{/each}
        </div></details>
      {/if}
      </div>
    {/if}
...
{#if metricsCard && metricsAnchor}
  <div class="metrics-popup" role="dialog" aria-label={`Jev classifications for ${metricsCard.subject || 'email'}`} use:floating={{anchor: metricsAnchor, side: 'right'}}>
    <header><span><strong>Jev categorisation</strong><small>{metricsCard.subject || '(no subject)'}</small></span><button aria-label="Close Jev classifications" onclick={closeMetrics}><X size={14}/></button></header>
    <div class="classification-list">
      <div><span>Importance</span><strong>{label(metricsCard.importance)}</strong><em>{metric(metricsCard.classificationMetrics?.importanceConfidence)}</em></div>
      <div><span>Intent</span><strong>{label(metricsCard.intent)}</strong><em>{metric(metricsCard.classificationMetrics?.intentConfidence)}</em></div>
      <div><span>Reply</span><strong>{label(metricsCard.replyRequired)}</strong><em>{metric(metricsCard.classificationMetrics?.replyConfidence)}</em></div>
      <div><span>Owner</span><strong>{label(metricsCard.suggestedOwner)}</strong><em>{metric(metricsCard.classificationMetrics?.ownerConfidence)}</em></div>
      <div><span>Next action</span><strong>{label(metricsCard.suggestedAction)}</strong><em>{metric(metricsCard.classificationMetrics?.actionConfidence)}</em></div>
    </div>
    <dl class="metrics-summary">
      <div><dt>Overall confidence</dt><dd>{metricsCard.confidence}% <small>lowest answer</small></dd></div>
      <div><dt>Priority score</dt><dd>{metricsCard.importanceScore}/100 <small>local mapping</small></dd></div>
      <div><dt>Inbox group</dt><dd>{inboxGroup(metricsCard)}</dd></div>
    </dl>
    <footer><span>{batch.classifier.provider || 'TypeSafe'} · {batch.classifier.model || 'model not reported'}</span><span>{batch.classifier.latencyMs ? `${batch.classifier.latencyMs}ms` : 'latency not reported'} · {batch.classifier.inputTokens ?? '—'} in · {batch.classifier.outputTokens ?? '—'} out · {cost(batch.classifier.costMicrousd)}</span></footer>
  </div>
{/if}
