...
<section class="policy"><h3>Remembered phones</h3><label>Inactivity limit (days)<input inputmode="numeric" bind:value={policyInput} placeholder="Indefinite" disabled={!record||saving}/></label><small>Blank means access remains until you revoke it. Each authenticated access renews the full number of days. For example, 14 days starts again whenever the phone connects or uses the desktop.</small><button onclick={savePolicy} disabled={!record||saving}>Save policy</button></section>
