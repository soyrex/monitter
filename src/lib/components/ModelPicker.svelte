<script lang="ts">
  import {Check,ChevronDown,LoaderCircle,RefreshCw,X,Zap} from '@lucide/svelte';
...
<div class="model-picker" bind:this={root}>
  <button class="model-trigger" bind:this={anchor} aria-label={`Model: ${label}`} aria-haspopup="dialog" aria-expanded={open} title={disabled?'Model can be changed when this run finishes':'Choose model and reasoning effort'} onclick={()=>{open=!open;query='';}}>
    {#if loading||saving}<LoaderCircle size={13} class="spin"/>{:else if current.fastMode && selected?.supportsFast}<Zap size={13}/>{/if}<span>{label}</span><ChevronDown size={12}/>
  </button>
...
<style>
  .model-picker{position:relative;min-width:0}.model-trigger{display:flex;align-items:center;gap:6px;color:var(--muted);padding:5px 7px;border-radius:6px;max-width:220px;font-size:11px}.model-trigger:hover{background:var(--soft);color:var(--ink)}.model-trigger span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .model-menu{position:fixed;inset:auto;margin:0;padding:12px;width:330px;max-height:min(520px,80vh);overflow:auto;border:1px solid var(--line);border-radius:12px;background:var(--panel);color:var(--ink);box-shadow:0 12px 40px #0004;font-family:inherit;font-size:12px}.model-menu header{display:flex;align-items:center;gap:4px;margin-bottom:10px}.model-menu header strong{flex:1;font-weight:500}.model-menu header button{display:grid;place-items:center;width:25px;height:25px;border-radius:5px;color:var(--muted)}.model-menu button:hover{background:var(--soft)}.model-menu input:not([type]){width:100%;padding:7px;margin-bottom:8px}.model-list{max-height:245px;overflow:auto;overscroll-behavior:contain}.model-list button{display:flex;align-items:center;gap:10px;width:100%;padding:9px;border-radius:7px;text-align:left}.model-list button>span{display:grid;gap:4px;flex:1}.model-list b{font-size:12px;font-weight:500}.model-list small,.effort small,.fast-row small{display:block;color:var(--muted);font-size:11px;line-height:1.4}.model-list .chosen{background:var(--soft)}.model-list :global(svg){color:var(--accent-ink);flex-shrink:0}.explanation,.error{font-size:11px;line-height:1.5;margin:5px 0 10px;color:var(--muted)}.error{color:var(--danger,#c44c79)}.effort{border-top:1px solid var(--line);margin-top:10px;padding-top:12px}.effort label{display:flex;justify-content:space-between;gap:8px}.effort label b{text-transform:capitalize;font-weight:500}.effort input{width:100%;margin:12px 0 6px;accent-color:var(--accent)}.fast-row{display:flex;align-items:center;gap:10px;margin-top:14px}.fast-row>span{flex:1}.fast-row b{display:block;font-weight:500;margin-bottom:4px}.fast-row input{appearance:none;flex-shrink:0;width:30px;height:18px;padding:0;border:0;border-radius:12px;background:var(--muted);position:relative;cursor:pointer}.fast-row input:checked{background:var(--accent)}.fast-row input:before{content:"";position:absolute;width:14px;height:14px;left:2px;top:2px;border-radius:50%;background:white;transition:transform .12s}.fast-row input:checked:before{transform:translateX(12px)}.reset{margin-top:12px;color:var(--muted);font-size:11px;text-align:left;padding:5px;border-radius:5px}
</style>
