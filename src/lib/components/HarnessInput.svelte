<script lang="ts">
  ...
  let { input, disabled = false, onsubmit }: { input: InteractionInput; disabled?: boolean; onsubmit: (value: unknown) => void } = $props();
...
<form class="harness-input" onsubmit={submit}>
  {#if input.kind === 'questions'}
  ...
  <button type="submit" disabled={disabled || !supported || (input.kind === 'url' && !safeUrl)}>{input.kind === 'url' ? 'I have completed this step' : 'Submit response'}</button>
</form>
