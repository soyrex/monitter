<script lang="ts">
  ...
  const newSkill = (): ManagedSkill => ({ id: crypto.randomUUID(), name: '', description: '', enabled: true, agentIds: [], content: '' });
  ...
  function agentSummary(ids: string[]) { return ids.length ? `${ids.length} assigned` : 'No agents'; }
...
<div class="section-stack extensions" aria-busy={loading}>
  ...
  {#if editingSkill}<section class="setting-card editor" aria-labelledby="skill-editor"><div class="inline-heading"><strong id="skill-editor">{config.skills.some(item => item.id === editingSkill?.id) ? 'Edit skill' : 'Add skill'}</strong><button type="button" onclick={() => editingSkill = null}>Cancel</button></div><label>Name<input bind:value={editingSkill.name}/></label><label>Description<input bind:value={editingSkill.description} placeholder="What this skill helps with"/></label><label>Markdown <small>Plain Markdown only, maximum 128 KiB.</small><textarea class="skill-content" bind:value={editingSkill.content}></textarea></label><fieldset><legend>Available to</legend>{#each agents as agent}<label class="agent-choice"><input type="checkbox" checked={editingSkill.agentIds.includes(agent.id)} onchange={() => editingSkill && (editingSkill.agentIds = selected(editingSkill.agentIds, agent.id))}/>{agent.name}</label>{:else}<p class="hint">No agents exist yet. Leaving this blank means no agent can use it.</p>{/each}</fieldset><button type="button" class="primary" disabled={saving} onclick={saveSkill}>{saving ? 'Saving…' : 'Save skill'}</button></section>{/if}
</div>
