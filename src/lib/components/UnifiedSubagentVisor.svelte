<script lang="ts">
  ...
  import { Brain, ChevronRight, LoaderCircle, Wrench, X } from '@lucide/svelte';
  ...
  function disclosure(value: string) {
    const [heading = 'Tool call', ...rest] = value.trim().split(/\r?\n/);
            ...
            <div class="transcript" aria-live="polite" aria-label={`${selected.agentName} transcript`}>
              {#if selected.transcript.length}
                {#each selected.transcript as entry (entry.id)}
                  {@const content = disclosure(entry.text)}
                  {#if entry.role === 'activity'}
                    <details class="tool-call">
                      <summary><ChevronRight class="disclosure-chevron" size={12}/><Wrench size={12}/><span>{content.heading}</span>{#if time(entry.createdAt)}<time>{time(entry.createdAt)}</time>{/if}</summary>
                      <div class="tool-detail">{content.detail || 'No additional details.'}</div>
                    </details>
                  {:else if entry.role === 'reasoning'}
                    <details class="thinking-block">
                      <summary><ChevronRight class="disclosure-chevron" size={12}/><Brain size={12}/><span>Thinking</span>{#if time(entry.createdAt)}<time>{time(entry.createdAt)}</time>{/if}</summary>
                      <div class="tool-detail">{entry.text}</div>
                    </details>
                  {:else}
...
<style>
  ...
  @container (max-width:450px){.visor-tabs-inner,.visor-panel-content{width:calc(100% - (2 * var(--density-composer-margin-inline,12px)))}.transcript article{max-width:96%}header small{display:none}}
