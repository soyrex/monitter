...
<style>
  .subagent-visor{container-type:inline-size;position:relative;z-index:4;width:100vw;margin-left:calc(50% - 50vw);height:42px;color:var(--ink);font-size:calc(12px * var(--interface-font-ratio,1));pointer-events:none}.visor-shell{position:absolute;left:0;right:0;bottom:0;display:flex;flex-direction:column;width:100%;max-height:min(58dvh,520px);overflow:hidden;border:1px solid var(--line);border-radius:0;background:var(--panel);box-shadow:0 5px 18px #0001;pointer-events:auto;transition:box-shadow var(--motion-navigation,180ms) var(--motion-ease,ease)}.open .visor-shell{box-shadow:0 -18px 44px #0004}
  .visor-tabs{order:0;min-height:40px;padding:5px 7px;overflow:hidden;background:color-mix(in srgb,var(--paper) 72%,var(--panel));scrollbar-width:thin}.visor-tabs-inner,.visor-panel-content{width:min(var(--chat-content-max-width,900px),calc(100% - (2 * var(--density-composer-margin-inline,12px))));box-sizing:border-box;margin-inline:auto}.visor-tabs-inner{display:flex;align-items:center;gap:5px;min-width:0;overflow-x:auto}.open .visor-tabs{border-bottom:1px solid var(--line)}.close{display:grid;place-items:center;flex:none;width:28px;height:28px;margin-left:auto;padding:0;border:0;border-radius:6px;background:transparent;color:var(--muted);cursor:pointer}.close:hover{background:var(--soft)}.close:focus-visible{outline:2px solid var(--accent);outline-offset:1px}
